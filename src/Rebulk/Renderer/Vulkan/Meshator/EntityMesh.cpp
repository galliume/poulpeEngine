...
namespace Rbk
{
    ...
    {
        for (int j = 0; j < mesh->GetData()->m_Ubos.size(); j++) {
            ...
            bbox->SetName("bbox_" + mesh->GetData()->m_Name);
            ...
            if (0 == count) {
                ...
                for (uint32_t i = 0; i < uniformBuffersCount; i++) {
                    ...
                    bbox->GetData()->m_UbosOffset.emplace_back(uboOffset);
                    Buffer uniformBuffer = m_Adapter->Rdr()->CreateUniformBuffers(nbUbo);
            ...
            for (uint32_t i = 0; i < mesh->m_UniformBuffers.size(); i++) {
                max = mesh->GetData()->m_UbosOffset.at(i);
                const auto ubos = std::vector<UniformBufferObject>(mesh->GetData()->m_Ubos.begin() + min, mesh->GetData()->m_Ubos.begin() + max);
