...
namespace Rebulk {
	...
	bool VulkanRenderer::DrawFrame(VkSwapchainKHR swapChain, std::vector<VkCommandBuffer> commandBuffers, std::pair<std::vector<VkSemaphore>, std::vector<VkSemaphore>> semaphores)
	{
		...
		std::cout << "DrawFrame vkQueueWaitIdle result " << std::to_string(result) << std::endl;
	...
	std::vector<VkImageView> VulkanRenderer::CreateImageViews()
	{
		...
		for (size_t i = 0; i < m_SwapChainImages.size(); i++) {
			...
			if (vkCreateImage(m_Device, &info, nullptr, &m_SwapChainImages[i]) != VK_SUCCESS) {
				throw std::runtime_error("failed to create image!");
			}
			
			VkMemoryRequirements memRequirements;
			vkGetImageMemoryRequirements(m_Device, m_SwapChainImages[i], &memRequirements);
			...
			vkBindImageMemory(m_Device, m_SwapChainImages[i], depthImageMemory, 0);
	...
	std::pair<VkPipeline, VkPipelineLayout> VulkanRenderer::CreateGraphicsPipeline(VkRenderPass renderPass, VkDescriptorSetLayout descriptorSetLayout)
	{
	...
	void VulkanRenderer::RecreateSwapChain() 
	{
		...
		std::pair<VkPipeline, VkPipelineLayout>pipeline = CreateGraphicsPipeline(m_RenderPass, descriptorSetLayout);
		m_GraphicsPipeline = pipeline.first;
		m_PipelineLayout = pipeline.second;
	...
	VkDescriptorSetLayout VulkanRenderer::CreateDescriptorSetLayout()
	{
		...
		samplerLayoutBinding.pImmutableSamplers = samplers;
	...
	std::vector<VkDescriptorSet> VulkanRenderer::CreateDescriptorSets(VkDescriptorPool descriptorPool, VkDescriptorSetLayout descriptorSetLayout, std::vector<VkImageView> imageViews)
	{
	...
	void VulkanRenderer::BeginRenderPass(VkRenderPass renderPass, std::vector<VkCommandBuffer> commandBuffers, std::vector<VkFramebuffer> swapChainFramebuffers, std::pair<VkPipeline, VkPipelineLayout>pipeline)
	{
		for (size_t i = 0; i < commandBuffers.size(); i++) {
			...
			TransitionLayout(commandBuffers[i], m_SwapChainImages[i], VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
		}
	}

	void VulkanRenderer::TransitionLayout(VkCommandBuffer commandBuffer, VkImage image, VkImageLayout oldLayout, VkImageLayout newLayout)
	{
	...
	void VulkanRenderer::EndRenderPass(std::vector<VkCommandBuffer> commandBuffers)
	{	
		for (size_t i = 0; i < commandBuffers.size(); i++) {
			vkCmdEndRenderPass(commandBuffers[i]);			

			TransitionLayout(commandBuffers[i], m_SwapChainImages[i], VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
