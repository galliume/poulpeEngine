...
//    }
    ...
    fColor = vec4(lights, 1.0f);
}

vec3 CalcDirLight(Light dirLight, vec3 normal, vec3 viewDir)
{
    vec3 lightDir = normalize(-dirLight.position);
    vec3 ambient = material.ambient * dirLight.ads.x * dirLight.color * texture(texSampler[0], fs_in.fTexCoord).xyz;

    float diff = max(dot(normal, lightDir), 0.0);
    vec3 diffuse = material.diffuse * diff * dirLight.ads.y * texture(texSampler[0], fs_in.fTexCoord).xyz;
    ...
    if (fs_in.fMapsUsed.y > 0.0) {
      ...
      specular = spec * dirLight.ads.z * material.specular * texture(texSampler[1], fs_in.fTexCoord).xyz;
    } else {
      float spec = pow(clamp(dot(normal, h), 0.0, 1.0), material.shiIorDiss.x) * float(dot(normal, h) > 0.0);
      specular = dirLight.ads.z * spec * material.specular;
    }
...
vec3 CalcPointLight(Light pointLight, vec3 normal, vec3 fragPos, vec3 viewDir)
{
    ...
    vec3 ambient = material.ambient * pointLight.ads.x * pointLight.color * texture(texSampler[0], fs_in.fTexCoord).xyz;
    ...
    float diff = max(dot(normal, lightDir), 0.0);
    vec3 diffuse = material.diffuse * diff * pointLight.ads.y * texture(texSampler[0], fs_in.fTexCoord).xyz;
    ...
    if (fs_in.fMapsUsed.y > 0.0) {
      float spec = pow(clamp(dot(normal, h), 0.0, 1.0), material.shiIorDiss.x) * float(dot(normal, h) > 0.0);
      specular = spec * pointLight.ads.z * material.specular * texture(texSampler[1], fs_in.fTexCoord).xyz;
    } else {
      float spec = pow(clamp(dot(normal, h), 0.0, 1.0), material.shiIorDiss.x) * float(dot(normal, h) > 0.0);
      specular = pointLight.ads.z * spec * material.specular;
    }
...
vec3 CalcSpotLight(Light spotlight, vec3 normal, vec3 fragPos, vec3 viewDir)
{
    ...
    vec3 ambient = material.ambient * spotlight.ads.x * spotlight.color * texture(texSampler[0], fs_in.fTexCoord).xyz;
    ...
    float diff = max(dot(normal, lightDir), 0.0);
    vec3 diffuse = material.diffuse * diff * spotlight.ads.y * texture(texSampler[0], fs_in.fTexCoord).xyz;
    ...
    if (fs_in.fMapsUsed.y > 0.0) {
        float spec = pow(clamp(dot(normal, h), 0.0, 1.0), material.shiIorDiss.x) * float(dot(normal, h) > 0.0);
        specular = spec * spotlight.ads.z * material.specular * texture(texSampler[1], fs_in.fTexCoord).xyz;
    } else {
        float spec = pow(clamp(dot(normal, h), 0.0, 1.0), material.shiIorDiss.x) * float(dot(normal, h) > 0.0);
        specular = spotlight.ads.z * spec * material.specular;
    }
