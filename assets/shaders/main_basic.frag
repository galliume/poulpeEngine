...
vec3 CalcDirLight(vec4 color, Light light, vec3 normal, vec3 view_dir, float shadow);
...
void main()
{
  ...
  vec3 color = CalcDirLight(texture_color, sun_light, normal, view_dir, shadow_ambient);
...
vec3 CalcDirLight(vec4 color, Light light, vec3 normal, vec3 view_dir, float shadow)
{
  vec3 light_dir = normalize(var.tangent_light_pos - var.tangent_frag_pos);
  vec3 ambient = (light.color * vec3(texture(texSampler[0], var.texture_coord))) * light.ads.x;

  float diff = max(dot(normal, light_dir), 0.0);
  vec3 diffuse =  (diff * vec3(texture(texSampler[0], var.texture_coord))) * light.ads.y;
  ...
  if (tex_size.x == 1 && tex_size.y == 1) {
    specular = (material.specular * spec) * light.color * light.ads.z;
  } else {
    specular = (vec3(texture(texSampler[2], var.texture_coord)) * spec) * light.color * light.ads.z;
  }
