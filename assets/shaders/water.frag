...
void main()
{
  vec3 deep_color = vec3(0.02, 0.05, 0.12); 
  vec3 shallow_color = vec3(0.3, 0.6, 0.65);
...
//  //@todo do PBR for water ?
  ...
  vec3 F0 = vec3(0.02);
  vec3 F90 = vec3(1.0);
  float metallic = 0.1; // Water is a dielectric
  float P = 1.0;
  float roughness = 0.2; // Lower roughness for sharper reflections
